public class Solution {
    ...
    private void DfsRecursive(char[][] grid, int r, int c, HashSet<(int row,int col)> visited){
        int ROWS = grid.Length; 
        int COLS = grid[0].Length;
        int[][] directions = [[1,0],[-1,0],[0,1],[0,-1]] ;

        if(r < 0 || c <0 || r>=ROWS || c >= COLS || grid[r][c] != '1' || visited.Contains((r,c))) return;
        visited.Add((r,c));

        foreach(var direction in directions){
                int nr = r + direction[0]; int nc = c + direction[1];               
                DfsRecursive(grid,nr,nc,visited);
            }
